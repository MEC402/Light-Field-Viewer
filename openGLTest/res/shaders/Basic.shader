...
vec4 biLerp(float x, float y, float x1, float x2, float y1, float y2)
{
...
vec4 getColorOnST(vec2 stCoords)
{
	...
	vec4 b = biLerp(v_uv.x, v_uv.y, u1, u2, v1, v2);
	float b11 = b.x; 
	float b12 = b.y; 
	float b21 = b.z; 
	float b22 = b.w;
...
void main() {
	...
	if (v_uv.x == -1 && v_uv.y == -1) {
		...
		if (v_st.x == -1 && v_st.y == -1)
		{
			...
			//Interpolate the color using bilienar interpolation factors.
			vec4 b_st = biLerp(v_st.x, v_st.y, s1, s2, t1, t2);
			float b_st11 = b_st.x;
			float b_st12 = b_st.y;
			float b_st21 = b_st.z;
			float b_st22 = b_st.w;

			//texpos.x = (v_texCoord.x + ceil(v_uv.x)) / 9;
			//texpos.y = (v_texCoord.y + ceil(v_uv.y)) / 9;
